...
(function () {
    ...
    function renderSessionControl() {
        ...
        if (!control) {
            ...
            control.innerHTML = `
                <div class="asada-session-identity">
                    ...
                    <small data-asada-session-email></small>
                </div>
        ...
        control.querySelector("[data-asada-session-email]").textContent = state.user.email || "";
    ...
    function logoutSession() {
        const message = navigator.onLine
            ? "¿Desea cerrar la sesión? Los borradores se conservarán."
    ...
    function setModalContent(mode, snapshot, savedRecord) {
        ...
        if (mode === "confirm") {
            ...
            message.textContent = "¿Está seguro de eliminar este registro? Esta acción quitará la información del listado y no se puede deshacer.";
            confirmButton.hidden = false;
            ...
            confirmButton.hidden = true;
            closeButton.textContent = "Cerrar";
            newButton.hidden = Boolean(snapshot.editing);
            newButton.textContent = "Crear otro registro";
        }

        modal.dataset.asadaPreviewKind = snapshot.kind || "";
    ...
    function initializeAuthenticatedUi() {
        ...
        if (isSharedWorker(state.user) && !state.operatorKey) {
