...
(function () {
    ...
    function installApiGuard() {
        ...
        const guardedCallApi = async function (action, values) {
            ...
            try {
                ...
                if (
                    ["saveRecord", "saveAccident", "saveCloro", "saveAforo"].includes(action) &&
                ...
                if (["saveRecord", "saveAccident", "saveCloro", "saveAforo"].includes(action)) {
                    state.pendingSubmission = null;
                }
    ...
    function installBulkDeleteToolbar() {
        ...
        // No mostrar la barra de eliminación mientras el listado está vacío,
        // cargando o únicamente contiene el estado "sin registros".
        // Se elimina del DOM para que tampoco ocupe espacio visual.
        if (!checkboxes.length) {
            document.querySelector("[data-asada-bulk-toolbar]")?.remove();
            return;
        }
