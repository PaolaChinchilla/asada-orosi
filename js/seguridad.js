...
(function () {
    ...
    function getSessionProfile(user) {
        ...
        if (isSharedWorker(user)) {
            return {
                name: getOperatorLabel(state.operatorKey) || "Departamento de Campo",
                role: "Trabajador"
            };
        }
    ...
    async function initializeDraftsPage() {
        ...
        root.innerHTML = `
            <section class="asada-drafts-hero">
                <span class="asada-access-label">DEPARTAMENTO DE CAMPO</span>
        ...
        const entries = await Promise.all(draftTypes.map(async kind => ({
            kind,
            draft: await getLocalDraft(kind)
        })));
        const available = entries.filter(entry => entry.draft && entry.draft.datos);
