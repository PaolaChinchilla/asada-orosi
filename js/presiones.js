...
(function () {
    ...
    function formatPressureMca(value) {
        const psi = value === "" || value === null || value === undefined ? NaN : Number(value);
        return Number.isFinite(psi) ? (psi / MCA_FACTOR).toFixed(2) : "—";
    }
    ...
    function buildRows(form, existingRows) {
        ...
        tbody.innerHTML = PRESSURE_POINTS.map((point, index) => {
            ...
            const mca = psi === "" || !Number.isFinite(Number(psi)) ? "" : (Number(psi) / MCA_FACTOR).toFixed(2);
            return `
                <tr>
                    ...
                    <td><input type="time" name="hora_${index}" aria-label="Hora de ${escapeHtml(point.lugar)}" required value="${escapeHtml(clean(row.hora))}"></td>
    ...
    function setPointCoordinates(form, index, coordinates) {
        ...
        if (summary) {
            summary.textContent = formatCoordinates(fields);
    ...
    function captureCoordinates(form, index, button) {
        ...
        const coordinateSummary = form.querySelector(`[data-presion-coordinate-index="${index}"]`);
        if (coordinateSummary) coordinateSummary.hidden = true;
        if (!navigator.geolocation) {
            if (coordinateSummary) coordinateSummary.hidden = false;
    ...
    function reportRows(record) {
        return (Array.isArray(record.mediciones) ? record.mediciones : []).map(row => [
            ...
            formatPressureMca(row.presionPsi),
            clean(row.hora),
    ...
    async function createPressurePdf(record) {
        ...
        const membreteWidth = 210;
        const membreteHeight = membreteWidth / (699 / 115);
        doc.addImage(letterhead, "PNG", (width - membreteWidth) / 2, 7, membreteWidth, membreteHeight);

        doc.setTextColor(30, 32, 36);
        doc.setFont("times", "normal");
        doc.setFontSize(17);
        doc.text("Hoja de registro de presiones", width / 2, 46, { align: "center" });
        doc.setFont("times", "normal");
        doc.setFontSize(11);
        doc.text(`Fecha y hora del control: ${formatDate(record.fecha)} · ID ${record.id}`, width / 2, 53, { align: "center" });
        doc.setDrawColor(19, 128, 112);
        doc.line(18, 59, width - 18, 59);

        const detailLine = (label, value, y) => {
            doc.setFont("helvetica", "bold");
            doc.setFontSize(11);
            const labelText = `${label}:`;
            doc.text(labelText, 18, y);
            const valueX = 18 + doc.getTextWidth(labelText) + 5;
            doc.setFont("helvetica", "normal");
            const lines = doc.splitTextToSize(
                String(value || "No indicado"),
                Math.max(30, width - 18 - valueX)
            );
            doc.text(lines, valueX, y);
            return Math.max(7, lines.length * 7);
        };
        let detailsHeight = detailLine("Responsable", record.responsable, 68);
        detailsHeight += detailLine("Ingresado por", record.ingresadoPor, 68 + detailsHeight);

        doc.autoTable({
            startY: 68 + detailsHeight + 4,
            head: [["Ruta", "Lugar de medición", "Presión PSI", "Presión MCA", "Hora", "Coordenadas"]],
    ...
    function detailTable(record) {
        ...
        const headers = ["Ruta", "Lugar de medición", "Presión PSI", "Presión MCA", "Hora", "Coordenadas"];
    ...
    async function bindForm() {
        ...
        retryButton?.addEventListener("click", async () => {
            ...
            try {
                ...
                if (typeof window.asadaShowPostSavePreview === "function") {
                    window.asadaShowPostSavePreview("presion", pendingSavedRecord);
                }
            } catch (error) {
        ...
        form.addEventListener("submit", async event => {
            ...
            try {
                const recordId = clean(form.querySelector('[name="id"]').value);
                if (typeof window.asadaShowSavingDialog === "function") {
                    window.asadaShowSavingDialog(
                        "presion",
                        Boolean(recordId),
                        "Estamos guardando las lecturas y el reporte. Espere un momento."
                    );
                }
                ...
                if (typeof window.asadaShowPostSavePreview === "function") {
                    window.asadaShowPostSavePreview("presion", saved);
                } else {
                ...
                if (sheetSaved) {
                    ...
                    if (typeof window.asadaShowErrorDialog === "function") {
                        window.asadaShowErrorDialog("presion", `El control ${pendingSavedRecord.id} sí quedó guardado en Sheets, pero el reporte PDF no se pudo guardar. Cierre este aviso y use «Reintentar guardado del PDF». ${formatApiError(error, "")}`);
                    }
                } else {
                    ...
                    if (typeof window.asadaShowErrorDialog === "function") window.asadaShowErrorDialog("presion", error);
                }
            } finally {
    ...
    window.ASADA_APPLY_PRESION_DRAFT = function (draft) {
        const form = document.getElementById("presionForm");
        if (!form || !draft) return;
        const rows = Array.isArray(draft.mediciones)
            ? draft.mediciones
            : parseJson(draft.medicionesJson, []);
        const hidden = form.querySelector('[name="medicionesJson"]');
        if (hidden) hidden.value = JSON.stringify(Array.isArray(rows) ? rows : []);
        buildRows(form, Array.isArray(rows) ? rows : []);
    };

    window.ASADA_RESET_PRESION_FORM = function (form) {
        if (!form) return;
        form.reset();
