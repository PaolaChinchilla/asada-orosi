(function () {
        ...
        form.addEventListener(
            ...
            async function (
                ...
                if (typeof window.asadaShowSavingDialog === "function") {
                    const editing = Boolean(new URLSearchParams(window.location.search).get("id"));
                    window.asadaShowSavingDialog(
                        "factibilidad",
                        editing,
                        "Estamos guardando la inspección y su fotografía. Espere un momento."
                    );
                }

                factibilidadSetStatus(
                    "Guardando factibilidad...",
                    "loading"
                );
                ...
                error
                    ...
                    if (typeof window.asadaShowErrorDialog === "function") {
                        window.asadaShowErrorDialog("factibilidad", error);
                    }
                }
            }
        );
    ...
    window.ASADA_RESET_FACTIBILIDAD_FORM = function (form) {
        if (!form) return;
        form.reset();
        factibilidadState.newImage = null;
        factibilidadState.existingImageSource = "";
        ...
        window.ASADA_FACTIBILIDAD_DRAFT_IMAGE = null;
        factibilidadClearNewImage();
        form.querySelectorAll("#factFormStatus, #factGpsStatus").forEach(status => {
            status.textContent = "";
            status.classList.remove("is-error", "is-success", "is-loading");
        });
        const title = document.querySelector(".page-head h1");
        if (title) title.textContent = "Crear inspección de factibilidad";
    };
    ...
    async function factibilidadGeneratePdf(record) {
        ...
        const letterhead = await factibilidadUrlToData("img/membrete-asada.png")
            .catch(() => "");
        ...
        const addFirstPageHeader = () => {
            doc.addImage(letterhead, "PNG", 16, 7, 178, 29.3);
