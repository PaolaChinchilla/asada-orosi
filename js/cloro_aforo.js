...
(function () {
    ...
    function renderModuleMap(mapRoot, latitude, longitude, label) {
        ...
        const coordinateCaption = mapRoot.parentElement?.querySelector(
            "[data-map-coordinates]"
        );

        if (mapRoot.__asadaLeafletMap) {
            mapRoot.__asadaLeafletMap.remove();
            mapRoot.__asadaLeafletMap = null;
        }

        if (!Number.isFinite(latitude) || !Number.isFinite(longitude)) {
            mapRoot.innerHTML = '<p class="module-map-note">Obtenga las coordenadas para visualizar el punto en el mapa.</p>';
            if (coordinateCaption) {
                coordinateCaption.textContent = "Punto aún no definido.";
            }
            return;
        }

        if (coordinateCaption) {
            coordinateCaption.textContent =
                `Punto: ${latitude.toFixed(7)}, ${longitude.toFixed(7)}`;
        }
        ...
        mapRoot.innerHTML = "";
        const map = window.L.map(mapRoot).setView([latitude, longitude], 17);
        ...
        const marker = window.L.marker([latitude, longitude], {
            ...
            .addTo(map)
            .bindPopup(label || "Ubicación capturada")
            .openPopup();

        if (form) {
            const setManualPoint = (nextLatitude, nextLongitude) => {
                const latitudeField = form.querySelector("[name=latitud]");
                const longitudeField = form.querySelector("[name=longitud]");
                if (!latitudeField || !longitudeField) {
                    return;
                }

                const nextLat = Number(nextLatitude);
                const nextLng = Number(nextLongitude);
                if (!Number.isFinite(nextLat) || !Number.isFinite(nextLng)) {
                    return;
                }

                latitudeField.value = nextLat.toFixed(7);
                longitudeField.value = nextLng.toFixed(7);
                marker.setLatLng([nextLat, nextLng]);
                map.setView([nextLat, nextLng], Math.max(map.getZoom(), 17));
                if (coordinateCaption) {
                    coordinateCaption.textContent = `Punto: ${nextLat.toFixed(7)}, ${nextLng.toFixed(7)}`;
                }
                setMessage(
                    form.querySelector("[data-gps-status]"),
                    "Punto actualizado manualmente en el mapa.",
                    "success"
                );
            };
        ...
        mapRoot.__asadaLeafletMap = map;
        // Leaflet puede inicializarse antes de que el contenedor termine de
        // pintarse, especialmente en teléfono. Estas dos actualizaciones
        // evitan que el mapa quede negro o en blanco.
