...
(function () {
    ...
    function bindAforoForm() {
        ...
        form.addEventListener("submit", async event => {
            ...
            if (missingMeasurement) {
                setMessage(status, `Capture las coordenadas del punto «${missingMeasurement.lugar}» antes de guardar.`, "error");
                return;
            }
            const id = valueFromForm(form, "[name=id]");
            const record = {
            ...
            try {
                ...
                if (!saved || !clean(saved.id)) {
                    throw new Error("Google Apps Script no confirmó la fila guardada. Actualice la implementación de Apps Script y vuelva a intentarlo.");
                }
                setMessage(status, "Registro guardado correctamente.", "success");
                if (typeof window.asadaShowPostSavePreview !== "function") {
                    window.alert("El registro de aforo se guardó correctamente en Google Sheets.");
                }
            } catch (error) {
    ...
    async function downloadAforoPdf(record) {
        ...
        await createPdf(
            ...
            { orientation: "landscape", compact: true, columnStyles: { 0: { cellWidth: 58 }, 1: { cellWidth: 36 }, 2: { cellWidth: 36 }, 3: { cellWidth: 68 } } }
        );
    }
