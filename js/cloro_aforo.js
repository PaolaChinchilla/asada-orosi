...
(function () {
    ...
    function stripUnit(value) {
        return clean(value).replace(/\s*(?:mg\/l|ntu|l\/s)$/i, "").trim();
    }

    function withUnit(value, unit) {
        const numericText = stripUnit(value);
        return numericText ? `${numericText} ${unit}` : "";
    }
    ...
    function buildCloroRows(form) {
        ...
        tbody.innerHTML = sites.map((site, index) => `
            <tr>
                ...
                <td><div class="module-unit-input"><input type="number" min="0" step="any" name="turbiedad_${index}" inputmode="decimal" aria-label="Turbiedad (NTU) de ${escapeHtml(site)}"><span>NTU</span></div></td>
                <td><div class="module-unit-input"><input type="number" min="0" step="any" name="cloro_${index}" inputmode="decimal" aria-label="Cloro (mg/L) de ${escapeHtml(site)}"><span>mg/L</span></div></td>
                <td><input type="number" min="0" max="14" step="any" name="ph_${index}" inputmode="decimal" aria-label="pH de ${escapeHtml(site)}"></td>
                <td><input name="olor_${index}" aria-label="Olor de ${escapeHtml(site)}"></td>
                <td><input type="number" min="0" step="any" name="temperatura_${index}" inputmode="decimal" aria-label="Temperatura de ${escapeHtml(site)}"></td>
        ...
        sites.forEach((site, index) => {
            ...
            ["turbiedad", "cloro", "ph", "olor", "temperatura", "hora", "latitud", "longitud", "altitud", "precisionGps"].forEach(key => {
                const input = form.querySelector(`[name="${key}_${index}"]`);
                if (input) {
                    input.value = ["turbiedad", "cloro", "ph", "temperatura"].includes(key)
                        ? stripUnit(row[key])
                        : clean(row[key]);
                }
            });
    ...
    function syncCloroHidden(form) {
        ...
        const rows = sites.map((lugar, index) => ({
            lugar,
            turbiedad: stripUnit(valueFromForm(form, `[name="turbiedad_${index}"]`)),
            cloro: stripUnit(valueFromForm(form, `[name="cloro_${index}"]`)),
            ph: valueFromForm(form, `[name="ph_${index}"]`),
            olor: valueFromForm(form, `[name="olor_${index}"]`),
            temperatura: stripUnit(valueFromForm(form, `[name="temperatura_${index}"]`)),
    ...
    function buildAforoRows(form) {
        ...
        tbody.innerHTML = sites.map((site, index) => `
            <tr>
                ...
                <td><div class="module-unit-input"><input type="number" min="0" step="any" name="litrosSegundo_${index}" inputmode="decimal" aria-label="Litros por segundo (L/s) de ${escapeHtml(site)}"><span>L/s</span></div></td>
                <td class="sample-coordinate-cell">
        ...
        sites.forEach((site, index) => {
            ...
            ["caudalAforado", "litrosSegundo"].forEach(key => {
                const input = form.querySelector(`[name="${key}_${index}"]`);
                if (input) {
                    input.value = key === "litrosSegundo" ? stripUnit(row[key]) : clean(row[key]);
                }
            });
            [["latitud", "latitudAforo"], ["longitud", "longitudAforo"], ["altitud", "altitudAforo"], ["precisionGps", "precisionGpsAforo"]].forEach(([key, name]) => {
                const input = form.querySelector(`[name="${name}_${index}"]`);
    ...
    function syncAforoHidden(form) {
        ...
        const rows = sites.map((lugar, index) => ({
            ...
            litrosSegundo: stripUnit(valueFromForm(form, `[name="litrosSegundo_${index}"]`)),
    ...
    function fillCoordinates(form, sampleIndex) {
        ...
        const coordinateLabel = form.querySelector(`[data-sample-coordinates-index="${targetSampleIndex}"]`);
        if (coordinateLabel) coordinateLabel.hidden = true;
        if (!navigator.geolocation) {
            if (coordinateLabel) coordinateLabel.hidden = false;
    ...
    function fillAforoCoordinates(form, index) {
        ...
        const coordinateLabel = form.querySelector(`[data-aforo-coordinates-index="${index}"]`);
        if (coordinateLabel) coordinateLabel.hidden = true;
        if (!navigator.geolocation) {
            if (coordinateLabel) coordinateLabel.hidden = false;
            setMessage(status, "Este dispositivo no permite obtener coordenadas.", "error");
    ...
    function bindCloroForm() {
        ...
        form.addEventListener("submit", async event => {
            ...
            if (!confirmedBySharedDialog) {
                if (typeof window.asadaRequestFormConfirmation === "function") {
                    window.asadaRequestFormConfirmation(form);
                }
                return;
            }
            ...
            try {
                ...
                if (typeof window.asadaShowSavingDialog === "function") {
                    window.asadaShowSavingDialog(
                        "cloro",
                        Boolean(id),
                        "Estamos guardando el control y su reporte. Espere un momento."
                    );
                }
                ...
                if (savedSuccessfully && confirmedBySharedDialog && typeof window.asadaShowPostSavePreview === "function") {
                    window.asadaShowPostSavePreview("cloro", savedRecord, null, message);
                } else {
                    if (typeof window.asadaHideSavingDialog === "function") window.asadaHideSavingDialog();
                    if (typeof window.asadaShowErrorDialog === "function") {
                        window.asadaShowErrorDialog("cloro", error);
            ...
            try {
                ...
                if (typeof window.asadaShowSavingDialog === "function") {
                    window.asadaShowSavingDialog(
                        "aforo",
                        Boolean(id),
                        "Estamos guardando el aforo y su reporte. Espere un momento."
                    );
                }
                ...
                setMessage(status, message, "error");
                if (savedSuccessfully && confirmedBySharedDialog && typeof window.asadaShowPostSavePreview === "function") {
                    window.asadaShowPostSavePreview("aforo", savedRecord, null, message);
                } else {
                    if (typeof window.asadaHideSavingDialog === "function") window.asadaHideSavingDialog();
                    if (typeof window.asadaShowErrorDialog === "function") {
                        window.asadaShowErrorDialog("aforo", error);
                    } else if (typeof window.alert === "function") {
                        window.alert(message);
                    }
                }
    ...
    function renderCloroDetail(record) {
        ...
        root.innerHTML = `
            ...
            <section class="detail-card module-detail-card"><h2>Puntos de muestreo</h2>${detailTable(samples.map(row => [row.lugar, withUnit(row.turbiedad, "NTU"), withUnit(row.cloro, "mg/L"), row.ph, row.olor, row.temperatura, row.hora, formatSampleCoordinates(row)]), ["Lugar", "Turbiedad (NTU)", "Cloro (mg/L)", "pH", "Olor", "Temperatura", "Hora", "Coordenadas"])}</section>
    ...
    function renderAforoDetail(record) {
        ...
        root.innerHTML = `
            ...
            <section class="detail-card module-detail-card"><h2>Puntos aforados</h2>${detailTable(measurements.map(row => [row.lugar, row.caudalAforado, withUnit(row.litrosSegundo, "L/s"), formatSampleCoordinates(row)]), ["Punto", "Caudal aforado", "Litros por segundo (L/s)", "Coordenadas"])}</section>
    ...
    async function createPdf(title, subtitle, rows, tableHeaders, filename, coordinateRows, options = {}) {
        ...
        const titleFontFamily = options.titleFontFamily || "helvetica";
        const subtitleFontFamily = options.subtitleFontFamily || titleFontFamily;
        doc.setFont(titleFontFamily, options.titleFontStyle || "normal");
        doc.setFontSize(Number(options.titleFontSize) || 16);
        const titleTextColor = Array.isArray(options.titleTextColor) && options.titleTextColor.length === 3
            ? options.titleTextColor.map(value => Math.max(0, Math.min(255, Number(value) || 0)))
            : [30, 32, 36];
        doc.setTextColor(...titleTextColor);
        doc.text(title, pageWidth / 2, Number(options.titleY) || 52, { align: "center" });
        doc.setTextColor(30, 32, 36);
        doc.setFont(subtitleFontFamily, options.subtitleFontStyle || "normal");
        ...
        coordinateRows.forEach(pair => {
            ...
            doc.text(label, 18, y);
            const valueX = 18 + doc.getTextWidth(label) + (Number(options.detailValueGap) || 5);
            doc.setFont(undefined, "normal");
            const value = pair[1] === "" || pair[1] === null || pair[1] === undefined
                ? "No indicado"
                : pair[1];
            const availableWidth = Math.max(30, pageWidth - 18 - valueX);
            const valueLines = doc.splitTextToSize(String(value), availableWidth);
            doc.text(valueLines, valueX, y);
            y += Math.max(detailSpacing, valueLines.length * detailSpacing);
        });
    ...
    async function downloadCloroPdf(record, settings = {}) {
        const rows = (record.muestras || []).map(row => [row.lugar, withUnit(row.turbiedad, "NTU"), withUnit(row.cloro, "mg/L"), row.ph, row.olor, row.temperatura, row.hora, formatSampleCoordinates(row)]);
        return await createPdf(
            "Control operativo de cloro residual",
            `${record.zona} · ${formatDate(record.fechaMuestreo)} · ID ${record.id}`,
            rows,
            ["Lugar", "Turbiedad (NTU)", "Cloro (mg/L)", "pH", "Olor", "Temperatura", "Hora", "Coordenadas"],
            `Reporte_Cloro_${record.id}.pdf`,
            [["Quién realiza", record.perfil], ["Responsable de campo", record.responsableCampo]],
            {
    ...
    async function downloadAforoPdf(record, settings = {}) {
        const rows = (record.mediciones || []).map(row => [row.lugar, row.caudalAforado, withUnit(row.litrosSegundo, "L/s"), formatSampleCoordinates(row)]);
        return await createPdf(
            "Hoja de registro de aforos",
            `${record.zona} · ${formatDate(record.fecha)} · ID ${record.id}`,
            rows,
            ["Punto", "Caudal aforado", "Litros por segundo (L/s)", "Coordenadas"],
            `Reporte_Aforo_${record.id}.pdf`,
            [["Condición del clima", record.condicionClima], ["Responsable de campo", record.responsableCampo], ["Ingresado por", record.ingresadoPor]],
            {
                orientation: "landscape",
                driveModule: "Aforos",
                recordId: record.id,
    ...
    window.ASADA_RESET_CLORO_FORM = function (form) {
        if (!form) return;
        form.reset();
    ...
    window.ASADA_RESET_AFORO_FORM = function (form) {
        if (!form) return;
        form.reset();
        const id = form.querySelector("[name=id]");
        if (id) id.value = "";
